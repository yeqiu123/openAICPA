...
public class GameView extends View {
    ...
    private int getHintCellPriority(int row, int col) {
        ...
        if (luckyClover[row][col] > 0) {
            // 幸运草会给多种随机奖励，智能提示优先指向可翻盘的好运点。
            priority += 15;
        }
        if (luckyStar[row][col] > 0) {
            // 幸运星会直接给随机道具，智能提示优先指向可补库存的资源点。
            priority += 15;
        }
        ...
        if (resonanceDrum[row][col] > 0) {
            // 共鸣鼓能直接进入爆发节奏，智能提示优先指向可打开连击窗口的落点。
            priority += 18;
        }
    ...
    private void drawHud(Canvas canvas) {
        ...
        if (level.energyPotionCount > 0) {
            ...
            if (lastEnergyPotionReward > 0) {
                obstacleText += " 充+" + lastEnergyPotionReward;
            }
        }
        ...
        if (level.hourglassCount > 0) {
            ...
            if (lastHourglassReward > 0) {
                obstacleText += " 步+" + lastHourglassReward;
            }
        }
        if (level.luckyCloverCount > 0) {
            // 幸运草会随机补金币、步数、能量或道具，HUD显示剩余和最新好运。
            obstacleText += " 草" + getLuckyCloverRemainingCount();
            if (lastLuckyCloverRewardType > 0) {
                obstacleText += " 好运";
            }
        }
        if (level.luckyStarCount > 0) {
            // 幸运星直接补随机道具，HUD显示剩余和本局最新道具。
            obstacleText += " 星" + getLuckyStarRemainingCount();
            if (lastLuckyStarRewardProp != NONE) {
                obstacleText += " 道具+1";
            }
        }
        ...
        if (level.resonanceDrumCount > 0) {
            // 共鸣鼓会开启爆发节奏，HUD单独显示剩余和本局爆发收益。
            obstacleText += " 鼓" + getResonanceDrumRemainingCount();
            if (lastResonanceDrumReward > 0) {
                obstacleText += " 爆+" + lastResonanceDrumReward;
            }
        }
        if (level.stardustJarCount > 0) {
            // 星尘罐会补能量和方向特效，HUD单独显示剩余和充能收益。
            obstacleText += " 尘" + getStardustJarRemainingCount();
            if (lastStardustJarReward > 0) {
                obstacleText += " 能+" + lastStardustJarReward;
            }
        }
        if (level.giftCount > 0) {
            // 礼盒会给随机资源，HUD单独显示剩余和本局礼盒收益。
            obstacleText += " 礼" + getGiftRemainingCount();
            if (lastGiftReward > 0) {
                obstacleText += " 奖+" + lastGiftReward;
            }
        }
        ...
        if (level.goldenEggCount > 0) {
            ...
            if (lastGoldenEggReward > 0) {
                obstacleText += " 金+" + lastGoldenEggReward;
            }
        }
    ...
    private String buildRecommendedPropTip(int prop) {
        ...
        } else if (getLuckyCloverRemainingCount() > 0
                && (prop == PROP_ROCKET || prop == PROP_LIGHTNING || prop == PROP_STAR_COMPASS || prop == PROP_HAMMER)) {
            return "推荐 " + getPropName(prop) + " 收幸运草拿好运";
        } else if (getLuckyStarRemainingCount() > 0
                && (prop == PROP_ROCKET || prop == PROP_LIGHTNING || prop == PROP_STAR_COMPASS || prop == PROP_HAMMER)) {
            return "推荐 " + getPropName(prop) + " 收幸运星拿道具";
        ...
        } else if (getGiftRemainingCount() > 0
                && (prop == PROP_ROCKET || prop == PROP_LIGHTNING || prop == PROP_STAR_COMPASS || prop == PROP_HAMMER)) {
            return "推荐 " + getPropName(prop) + " 开礼盒拿惊喜";
    ...
    private boolean isRecommendedPropForLevel(int prop) {
        ...
        if (getLuckyCloverRemainingCount() > 0
                && (prop == PROP_ROCKET || prop == PROP_LIGHTNING || prop == PROP_STAR_COMPASS || prop == PROP_HAMMER)) {
            // 幸运草奖励覆盖金币、步数、能量和道具，推荐精准道具优先收好运。
            return true;
        }
        if (getLuckyStarRemainingCount() > 0
                && (prop == PROP_ROCKET || prop == PROP_LIGHTNING || prop == PROP_STAR_COMPASS || prop == PROP_HAMMER)) {
            // 幸运星会直接补随机道具，推荐精准道具优先拿库存收益。
            return true;
        }
        ...
        if (getGiftRemainingCount() > 0
                && (prop == PROP_ROCKET || prop == PROP_LIGHTNING || prop == PROP_STAR_COMPASS || prop == PROP_HAMMER)) {
            // 礼盒能给随机补给，推荐精准道具优先打开基础奖励点。
            return true;
        }
    ...
    private String buildFailurePropAdviceText() {
        ...
        if (getLuckyCloverRemainingCount() > 0) {
            // 幸运草失败后提示优先收集，让随机好运更早进入下局节奏。
            return "建议下局优先收幸运草";
        }
        if (getLuckyStarRemainingCount() > 0) {
            // 幸运星失败后提示优先收集，让随机道具更早加入下局库存。
            return "建议下局优先收幸运星";
        }
        if (getMysteryBoxRemainingCount() > 0) {
    ...
    private void drawStone(Canvas canvas, int row, int col, RectF rect) {
        ...
        float pulse = 0.55f + 0.45f * (float) Math.sin(System.currentTimeMillis() / 255.0);
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(dp(2 + pulse * 2));
        paint.setColor(Color.argb((int) (65 + pulse * 75), 122, 130, 154));
        canvas.drawRoundRect(new RectF(rect.left + dp(5), rect.top + dp(5),
                rect.right - dp(5), rect.bottom - dp(5)), dp(13), dp(13), paint);
        paint.setStyle(Paint.Style.FILL);
    ...
    private void drawGift(Canvas canvas, int row, int col, RectF rect) {
        ...
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(dp(2 + pulse * 2));
        paint.setColor(Color.argb((int) (85 + pulse * 90), 255, 236, 133));
        canvas.drawRoundRect(new RectF(rect.left + dp(5), rect.top + dp(5),
                rect.right - dp(5), rect.bottom - dp(5)), dp(13), dp(13), paint);
        paint.setStyle(Paint.Style.FILL);
        paint.setColor(Color.argb(190, 255, 236, 133));
    ...
    private void drawChain(Canvas canvas, int row, int col, RectF rect) {
        ...
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(dp(2 + pulse * 2));
        paint.setColor(Color.argb((int) (75 + pulse * 80), 245, 245, 255));
        canvas.drawRoundRect(new RectF(rect.left + dp(5), rect.top + dp(5),
                rect.right - dp(5), rect.bottom - dp(5)), dp(13), dp(13), paint);
        paint.setStrokeWidth(dp(4));
    ...
    private void drawGoldenEgg(Canvas canvas, int row, int col, RectF rect) {
        ...
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(dp(2 + pulse * 2));
        paint.setColor(Color.argb((int) (85 + pulse * 90), 255, 196, 64));
        canvas.drawRoundRect(new RectF(rect.left + dp(5), rect.top + dp(5),
                rect.right - dp(5), rect.bottom - dp(5)), dp(13), dp(13), paint);
        paint.setStyle(Paint.Style.FILL);
    ...
    private void drawLuckyClover(Canvas canvas, int row, int col, RectF rect) {
        ...
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(dp(2 + pulse * 2));
        paint.setColor(Color.argb((int) (90 + pulse * 95), 80, 198, 126));
        canvas.drawRoundRect(new RectF(rect.left + dp(5), rect.top + dp(5),
                rect.right - dp(5), rect.bottom - dp(5)), dp(13), dp(13), paint);
        paint.setStyle(Paint.Style.FILL);
    ...
    private void drawLevelIntro(Canvas canvas) {
        ...
        if (level.giftCount > 0) {
            goalText += "  礼盒 " + level.giftCount;
            // 开场说明礼盒收益，让玩家优先打开随机补给点。
            goalText += "  开盒拿惊喜";
        }
        if (level.cloudCount > 0) {
    ...
    private String buildLevelStrategyHint(Level level) {
        ...
        } else if (level.giftCount > 0) {
            return "策略 火箭/罗盘优先开礼盒";
    ...
    private String buildFailureProgressText() {
        ...
        if (level.stoneCount > 0) {
            ...
            appendFailureProgressPart(text, "石块剩", stoneRemaining);
        }
        if (level.shellCount > 0) {
        ...
        if (level.luckyCloverCount > 0) {
            // 幸运草剩余量单独复盘，提示下局优先拿随机好运。
            appendFailureProgressPart(text, "幸运草剩", getLuckyCloverRemainingCount());
        }
        if (level.luckyStarCount > 0) {
            // 幸运星剩余量单独复盘，提示下局优先拿随机道具。
            appendFailureProgressPart(text, "幸运星剩", getLuckyStarRemainingCount());
        }
        if (level.mysteryBoxCount > 0) {
        ...
        if (level.giftCount > 0) {
            // 礼盒剩余量单独复盘，提示下局优先打开随机补给点。
            appendFailureProgressPart(text, "礼盒剩", getGiftRemainingCount());
        }
        if (level.cloudCount > 0) {
    ...
    private int getEnergyPotionRemainingCount() {
        int count = 0;
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                if (energyPotion[row][col] > 0) {
    ...
    private int getHourglassRemainingCount() {
        int count = 0;
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                if (hourglass[row][col] > 0) {
    ...
    private int getLuckyCloverRemainingCount() {
        int count = 0;
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                if (luckyClover[row][col] > 0) {
    ...
    private int getLuckyStarRemainingCount() {
        int count = 0;
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                if (luckyStar[row][col] > 0) {
    ...
    private int getResonanceDrumRemainingCount() {
        int count = 0;
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                if (resonanceDrum[row][col] > 0) {
    ...
    private int getGoldenEggRemainingCount() {
        int count = 0;
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                if (goldenEgg[row][col] > 0) {
    ...
    private int getGiftRemainingCount() {
        int count = 0;
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                if (gift[row][col] > 0) {
    ...
    private int getStardustJarRemainingCount() {
        int count = 0;
        for (int row = 0; row < BOARD_SIZE; row++) {
            for (int col = 0; col < BOARD_SIZE; col++) {
                if (stardustJar[row][col] > 0) {
